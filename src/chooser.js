...
export function recommend(raw){
 ...
 if(a.response==='yes') {
  ...
  return result(a.goal==='predict'?'まず基準になるモデルをつくり、未知のデータへの予測を比べます。':'ほかの要因をモデルに含めた上で、結果との関係を検討します。',candidate(baseline,repeated?'同じ対象・集団内の依存を考慮するためです。':a.outcome==='binary'?'結果が二値なので、確率をモデル化するためです。':'量的な結果と説明変数の関係をモデル化するためです。',`${a.outcome==='binary'&&repeated?'二項の一般化線形混合モデルを検討します。':''}説明変数の選び方、標本数、モデルの適合を確認します。${a.goal==='predict'?'予測時点で使える情報だけを使い、評価用データを分けます。':'係数を因果効果と解釈するには、研究設計と追加の仮定が必要です。'}`,a.outcome==='binary'&&repeated?'二項の混合効果モデル':undefined),...(a.goal==='predict'?[candidate('forest','曲がった関係や変数の組み合わせも捉える予測モデルとして、基準モデルと比較できます。',repeated?'同じ人や集団が学習側と評価側にまたがらない分割を考えます。森林自体が繰り返しの依存を解消するわけではありません。':'訓練に使ったデータでの精度だけで判断しません。少数例では、複雑なモデルを十分に検証できないことがあります。')]:[]));
 }
